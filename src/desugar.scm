...
(define (rewrite-cond cases)
  (cond ((not (pair? cases)) ''())
        ((or (eq? (caar cases) 'default) (eq? (caar cases) #t))
         (rewrite (cadar cases)))
        (#t
...
(define (rewrite-and tests)
  (if (pair? tests)
    `(if ,(rewrite (car tests)) ,(rewrite-and (cdr tests)) #f)
    '#t))
(define (rewrite-or tests)
  (if (pair? tests)
    `(if ,(rewrite (car tests)) #t ,(rewrite-or (cdr tests)))
    '#f))
...
(define (rewrite expr)
  (cond ((not (pair? expr)) expr)
        ...
        ((eq? (car expr) 'quasiquote) (rewrite-quasiquote (cadr expr) 0))
